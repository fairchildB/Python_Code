...
# Your code:
        ...
        try:
            # Get the bands by using the specified band numbers
            band4 = arcpy.Raster(self.raster_path + f"\\Band_{band4_index}")
            band3 = arcpy.Raster(self.raster_path + f"\\Band_{band3_index}")
...
#your code:
            ...
            print(f"Error calculating NDVI for raster at '{self.raster_path}': {e}")
            okay = False
            return okay, None
...
class SmartVectorLayer:
    ...
    def summarize_field(self, field):
        """Summarize a field by calculating its mean value."""
        okay = True
    ...
    def zonal_stats_to_field(self, raster_path, statistic_type="MEAN", output_field="ZonalStat"):
        ...
        try:
            ...
            # Calculate zonal statistics and store in the temporary table
            arcpy.sa.ZonalStatisticsAsTable(self.feature_class, "OBJECTID", raster_path, temp_table,
                                            "DATA", statistic_type)

            # Read the results from the temporary table
            zonal_results = {}
            with arcpy.da.SearchCursor(temp_table, ["OBJECTID_1", statistic_type]) as cursor:
                for row in cursor:
                    zonal_results[row[0]] = row[1]

            # Update the feature class with the zonal statistics
    ...
    def extract_to_pandas_df(self, fields=None):
        ...
        rows = (list(row) for row in arcpy.da.SearchCursor(self.feature_class, fields))
